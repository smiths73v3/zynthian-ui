...
class zynthian_gui_touchscreen_calibration:
    ...
    def __init__(self):
        ...
        # Crosshair target
        # Index of current calibration point (0=NW, 1=SE, 2=CENTRE)
        self.index = 2
    ...
    def detectDevice(self):
        ...
        while self.running and self.shown:
            # Wait for any of the devices to trigger an event
                        ...
                        if event.code == ecodes.BTN_TOUCH:
                            if event.value:
                                ...
                                if self.device_id:
                                    ...
                                    self.drawCross()
                                    self.canvas.bind(
                                        '<Button-1>', self.onPress)
                                    self.canvas.bind(
                                        '<ButtonRelease-1>', self.onRelease)
                                    self.running = False
    ...
    def setDevice(self, name, path):
        ...
        # Get node name to allow mapping between evdev and xinput names
        self.node = props[node_start:node_end]
        # Reset calibration to allow absolute acquisition
        self.setCalibration(self.device_id, [1, 0, 0, 0, 1, 0, 0, 0, 1])
        return True
            ...
            if abs(self.touch_points[0].x - self.touch_points[1].x) < self.debounce and abs(self.touch_points[0].y - self.touch_points[1].y) < self.debounce:
                ...
                if x0 < x1:
                    if y0 < y1:
                        ...
                        c = (0.15 * self.width / a - min_x) / \
                            self.width  # Offset to add to x-axis
                        d = 0  # Scaling factor of x-axis from pointer y coord
                        e = self.height * 0.7 / dy  # Scaling factor of y-axis from pointer y coord
                        f = (0.15 * self.height / e - min_y) / \
                            self.width  # Offset to add to y-axis
                    else:
                        # Rotated 90 CW
                        a = 0
    ...
    def drawCross(self):
        ...
        self.canvas.coords(self.crosshair_circle,
                           self.display_points[self.index].x - self.crosshair_size *
                           0.8, self.display_points[self.index].y -
                           self.crosshair_size * 0.8,
        ...
        self.canvas.coords(self.crosshair_inner_circle,
                           self.display_points[self.index].x - self.crosshair_size *
                           0.2, self.display_points[self.index].y -
                           self.crosshair_size * 0.2,
    ...
    def setCalibration(self, device, matrix, write_file=False):
        try:
            ...
            if write_file:
                ...
                try:
                    os.mkdir(os.environ.get(
                        "ZYNTHIAN_CONFIG_DIR") + "/touchscreen/")
                except:
    ...
    def build_view(self):
        ...
        self.canvas.itemconfig(self.countdown_text,
                               text="Closing in %ds" % (self.timeout))
    ...
    def onTimer(self):
        if self.shown:
            self.canvas.itemconfig(self.countdown_text,
                                   text="Closing in %ds" % (self.countdown))
            if self.countdown <= 0:
                self.zyngui.zynswitch_defered('S', 1)
                return
            if not self.pressed:
